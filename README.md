...
Fabric からリポジトリのルートを dbt プロジェクトとして認識できるように、`dbt_project.yml` はルートへ配置しています。モデル、Seed、テストなどのdbt資材は、Fabric Workspace の Git 連携で同期されるアイテムと混在しないように `src/dbt/` 配下へまとめています。

## データフロー

```text
src/dbt/seeds/customers.csv -> stg_customers -> dim_customers --+
                                                                +-> fct_orders
src/dbt/seeds/orders.csv    -> stg_orders ----------------------+
```
...
ローカルでの開発とテスト用に、Fabric接続プロファイルのひな型を `src/dbt/profiles.yml.example` に用意しています。これをdbtのプロファイルディレクトリへコピーし、接続先を変更してください。
